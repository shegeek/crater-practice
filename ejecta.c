...
{
  GLdouble r, theta;
  theta = (double)(rand() % 314) / 50.;
  r = (double)(rand() % 100) / 100. * maxradius;
  redux->dircos[0] = cos(theta);
  redux->dircos[1] = sin(theta);
  redux->initpos[0] = r * redux->dircos[0];
  redux->initpos[1] = 0.;
    redux->initpos[2] = r * redux->dircos[1];
    redux->initvelocity[0] = (double)(rand() % 200) / 100.;
  redux->initvelocity[1] = (double)(rand() % 200) / 100. + 8.;
  redux->initvelocity[2] = (double)(rand() % 200) / 100.;
...
bool advance_ejparticle(ejparticle * updated, double elapsed, double gconst)
{
  ...
  updated->currpos[0] = updated->initvelocity[0] * elapsed * 
    updated->dircos[0] + updated->initpos[0];
  ...
  updated->currpos[2] = updated->initvelocity[2] * elapsed * 
    updated->dircos[1] + updated->initpos[2];
  return islive;
}
